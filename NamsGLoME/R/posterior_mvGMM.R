...
posterior_mvGMM = function(X, gmm) {
  ...
  log_pik_fik <- mvGMM_pdf(X, gmm)$log_pik_fik
  ...
  log_sum_pik_fik <- logsumexp(log_pik_fik, 1)
  log_tauik <- log_pik_fik - log_sum_pik_fik %*% ones(1, K)
  #tau = exp(log_Tauik)
  tau <- exp(log_tauik) / (apply(exp(log_tauik), 1, sum) %*% ones(1, K))
  loglik <- sum(log_sum_pik_fik)
